	...
	: nh_private_("~")
{
    ...
    nh_private_.param("save_topic_suffix", save_topic_suffix_, std::string("16uc1"));
    ...
    for(size_t i = 0; ; i++){
        ...
        tmp_topic.debug_pub = nh_.advertise<sensor_msgs::Image>(tmp_topic.topic_name + "/" + save_topic_suffix_, 1);
        ...
        if(view_itr->getDataType() == "sensor_msgs/Image"){
            for(Topic& topic : topic_list_){
                if(view_itr->getTopic() == topic.topic_name){
                    ...
                    save_bag_.write(topic.topic_name + "/" + save_topic_suffix_, view_itr->getTime(), cv_16uc1.toImageMsg());

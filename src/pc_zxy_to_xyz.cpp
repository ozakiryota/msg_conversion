...
PcZxyToXyz::PcZxyToXyz()
	: nh_private_("~")
{
	std::cout << "--- pc_zxy_to_xyz ---" << std::endl;
	/*parameter*/
	nh_private_.param("publish_frame", publish_frame_, std::string(""));
	std::cout << "publish_frame_ = " << publish_frame_ << std::endl;
	/*subscriber*/
	pc_sub_ = nh_.subscribe("/point_cloud", 1, &PcZxyToXyz::callbackPC, this);
	/*publisher*/
...
void PcZxyToXyz::listUpPointType(void)
{
	_list_fields = {
		"x y z",
		"x y z intensity",
...
void PcZxyToXyz::callbackPC(const sensor_msgs::PointCloud2ConstPtr& msg)
{
...
void PcZxyToXyz::checkTypeAndConvert(const sensor_msgs::PointCloud2& sub_pc_msg, sensor_msgs::PointCloud2& pub_pc_msg)
{
...
void PcZxyToXyz::fixAxises(CloudPtr pc, const sensor_msgs::PointCloud2& sub_pc_msg, sensor_msgs::PointCloud2& pub_pc_msg)
{
...
void PcZxyToXyz::publishMsg(sensor_msgs::PointCloud2& pub_pc_msg)
{
...
{
    ros::init(argc, argv, "pc_zxy_to_xyz");
	
	PcZxyToXyz pc_zxy_to_xyz;
